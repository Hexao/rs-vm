...
impl CPU {
    ...
    pub fn get_register(&self, name: &'static str) -> Result<u8, MemoryError> {

        let reg_pointer = self.register_map.get(name).expect("Register name does not exist");
        self.registers.get_memory_at(*reg_pointer)
    }

    pub fn set_register(&mut self, name: &'static str, data: u8) -> Result<(), MemoryError> {

        let reg_pointer = self.register_map.get(name).expect("Register name does not exist");
        self.registers.set_memory_at(*reg_pointer, data)?;
